...
from TFAttention import TFAttentionModule
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, drop_rate=0.0,
                 dropblock_size=5, num_classes=-1, use_se=False, use_tfatt = False):
        ...
        self.layer4 = self._make_layer(block, n_blocks[3], 640,
                                       stride=2, drop_rate=drop_rate, drop_block=True, block_size=dropblock_size)
        if use_tfatt:
            self.tfatt1 = TFAttentionModule(64)
            self.tfatt2 = TFAttentionModule(160)
            self.tfatt3 = TFAttentionModule(320)
            self.tfatt4 = TFAttentionModule(640)

        self.use_tfatt = use_tfatt
        if avg_pool:
    ...
    def forward(self, x, is_feat=False, norm_feat=False,return_feature=False):
        x = self.layer1(x) 
        if self.use_tfatt:
            x = self.tfatt1(x)
        f0 = x
